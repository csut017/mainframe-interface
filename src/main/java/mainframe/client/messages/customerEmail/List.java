...
public class List implements Message {
    ...
    public Status send(Connection connection) throws IOException
    {
        ...
        if (response.getStatus().getWasSuccessful()) {
            ...
            for (int i = 1; i <= itemsCount; i++) {
                ...
                String addressValue = response.getValue(i + ":address");
                String isPrimaryValue = response.getValue(i + ":isPrimary");
                _data.add(new Item(addressValue, idValue, isPrimaryValue, typeValue));
            }
        }
    ...
    public class Item {
        ...
        public Item(String address, long id, String isPrimary, String type) {
            _address = address;
            _id = id;
